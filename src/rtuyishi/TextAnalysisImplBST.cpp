...
Node* insert(Node* root, const char* word, int& maxProbes, int& totalProbes, int& wordCount) {
    ...
    while (true) {
        ...
        // Checking if the words are same.
        if (cmp == 0) {
            current->frequency++;
...
// Strip apostrophe and all characters following it (e.g., "let's" -> "let", "cats'" -> "cats").
void stripApostropheAndBeyond(char* word) {
    for (int i = 0; word[i]; i++) {
        if (word[i] == '\'') {
            word[i] = '\0'; // Truncate at apostrophe
            break;
        }
    }
}
...
void lineProcessing(char* line, Node*& root, int& maxProbes, int& totalProbes, int& wordCount) {
    // Splitting the line into words, excluding apostrophe to preserve full words initially.
    char* token = strtok(line, " .,!?\";:()[]{}");
    while (token != NULL) {
        toLowerCase(token);
        // Strip apostrophe and beyond to handle possessives and contractions (e.g., "let's" -> "let", "cats'" -> "cats")
        stripApostropheAndBeyond(token);
        if (strlen(token) > 0) { // Only process non-empty tokens
